...
const GenerateImages = () => {
    const imageStyle = [
      ...
      const handleDownload = async (url, filename = "generated.png") => {
        try {
   ...
   <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        <div className='flex items-center gap-3'>
          <Sparkles className='w-6 text-[#00AD40]'/>
          <h1 className='text-xl font-semibold'>AI Image Generator</h1>
        </div>
        <p className='mt-6 text-sm font-medium'>Describe Your Image</p>
        <textarea rows={4} onChange={(e)=>setInput(e.target.value)} value={input} className='w-full p-2 px-3 mt-2 outline-none text-sm
        rounded-md border border-gray-300' placeholder='Desribe what type of Image you need...' required/>
        <p className='mt-4 text-sm font-medium'>Style</p>
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {imageStyle.map((item)=>(
            <span onClick={()=>setSelectedStyle(item)} key={item} className={`text-xs px-4 py-1 border rounded-full 
            cursor-pointer ${selectedstyle === item ? 'bg-green-50 text-green-700': 'text-gray-500 border-gray-300'}`}>{item}</span>
          ))}
        </div>
        ...
        from-[#00AD40] to-[#04FF60] text-white px-4 py-2 mt-6 text-sm rounded-lg cursor-pointer'>
          {
      ...
      border-gray-200 min-h-96'>
        ...
        <div className='flex-1 flex justify-center items-center'>
                  <div className='text-sm flex flex-col items-center gap-5 text-gray-400'>
                    ...
                    <p>Enter your prompt and click "Generate Image" to get started</p>
                  </div>
                </div>
          ) : (
            <div className="mt-3 h-full relative">
              <img src={content} alt="image" className="w-full h-full" />
              <Download onClick={()=>handleDownload(content)} className="absolute top-2 right-2 cursor-pointer w-6 h-6 text-white bg-black/50 rounded-full p-1"/>
            </div>
          )
        }
        </div>
