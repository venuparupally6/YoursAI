...
const Sidebar = ({sidebar, setSidebar}) => {
  ...
  return (
    ...
    items-center max-sm:absolute top-14 bottom-0 ${sidebar ? 'translate-x-0': 'max-sm:-translate-x-full'} transition-all duration-300
    ease-in-out z-10`}>
            ...
            <div className='px-6 mt-5 text-sm text-gray-600 font-medium'>
              {navItems.map(({to,label,Icon})=>(
                ...
                className={({isActive})=>`px-3.5 py-2.5 flex items-center gap-3 rounded
                 ${isActive? 'bg-gradient-to-r from-[#3C91F6] to-[#9234EA] text-white': ''}`}>
                  {({isActive})=>(
                    <>
